...
// uma CPU tem estado, MMU, controlador de ES
struct cpu_t {
  // registradores
  int PC;
  int A;
...
{
  // a validação de acesso fica a cargo da MMU
...
void cpu_executa_1(cpu_t *self)
{
  ...
  if (self->erro != ERR_OK && self->erro != ERR_CPU_PARADA) {
    if (!cpu_interrompe(self, IRQ_ERR_CPU)) {
      fprintf(stderr, "cpu_interrompe recusou: modo=%d erro=%d\n", self->modo, self->erro);
      assert(0);
    }
  }
}
